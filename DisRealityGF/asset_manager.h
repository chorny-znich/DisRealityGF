...
//	Realisation
	...
	*/
	...
	void AssetManager<Id, Asset>::init(const std::string& filename) {
		...
		if (!createAssetList(filename)) {
			throw std::runtime_error(std::format("Failed to open the file with the list of assets"));
		}
	...
	*/
	...
	bool AssetManager<Id, Asset>::createAssetList(const std::string& path) {
		...
		try {
			ini::Document doc = ini::Load(path);
			ini::Section section = doc.GetSection("assets");
